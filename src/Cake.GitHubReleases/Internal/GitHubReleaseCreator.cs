...
namespace Cake.GitHubReleases.Internal
{
    internal sealed class GitHubReleaseCreator
    {
        ...
        private void LogSettings(GitHubReleaseCreateSettings settings)
        {
        ...
        private async Task<Release?> TryGetReleaseAsync(IGitHubClient client, GitHubReleaseCreateSettings settings)
        {
            ...
            catch (NotFoundException)
            {
                ...
                var release = matchingReleases.Length switch
                {
                    0 => null,
                    1 => matchingReleases[0],
                    > 1 => throw new AmbiguousTagNameException($"There are multiple existing releases for tag name '{settings.TagName}'"),
                    < 0 => throw new InvalidOperationException()  // Cannot happen (.Count cannot be negative), but case is necessary to make switch cover all theoretical cases
                };
        ...
        private async Task DeleteReleaseAsync(IGitHubClient client, GitHubReleaseCreateSettings settings, Release release)
        {
